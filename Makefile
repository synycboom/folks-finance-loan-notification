...
prepare.env:
	...
	cd $(SRC_PATH)/services/monitor && cat .env.example > .env > .env.production
	cd $(SRC_PATH)/services/notifier && cat .env.example > .env > .env.production
	cd $(SRC_PATH)/services/user-setting && cat .env.example > .env > .env.production
...
.PHONY: deps-up deps-down monitor-up prepare.env prepare.node all-up
